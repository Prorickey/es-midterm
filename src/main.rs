...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let errors: Vec<String> = thread::scope(|s| {
        let handles: Vec<_> = vec![
            ...
            // No-Florida dataset
            s.spawn(|| scatter::generate(&no_fl_owned, &no_fl_bounds, "figures/no_fl_sightings_scatter.png", " (Excluding Florida)").map_err(|e| e.to_string())),
            s.spawn(|| seasonal_range::generate(&no_fl_owned, &no_fl_bounds, "figures/no_fl_seasonal_range.png", " (Excluding Florida)").map_err(|e| e.to_string())),
            s.spawn(|| monthly_heatmap::generate(&no_fl_owned, &no_fl_bounds, "figures/no_fl_monthly_heatmap.png", " (Excluding Florida)").map_err(|e| e.to_string())),
            s.spawn(|| nesting_latitude::generate(&no_fl_owned, "figures/no_fl_nesting_latitude.png", " (Excluding Florida)").map_err(|e| e.to_string())),
            s.spawn(|| monthly_latitude::generate(&no_fl_owned, "figures/no_fl_monthly_latitude.png", " (Excluding Florida)").map_err(|e| e.to_string())),
            s.spawn(|| yearly_monthly_latitude::generate(&no_fl_owned, "figures/no_fl_yearly_monthly_latitude.png", " (Excluding Florida)").map_err(|e| e.to_string())),
            s.spawn(|| julian_date::generate(&no_fl_owned, "figures/no_fl_julian_date.png", " (Excluding Florida)").map_err(|e| e.to_string())),
            s.spawn(|| state_bar::generate(&no_fl_owned, "figures/no_fl_state_observations.png", " (Excluding Florida)").map_err(|e| e.to_string())),
            s.spawn(|| breeding_map::generate(&no_fl_owned, &no_fl_bounds, "figures/no_fl_breeding_evidence.png", " (Excluding Florida)").map_err(|e| e.to_string())),
            s.spawn(|| breeding_latitude::generate(&no_fl_owned, "figures/no_fl_breeding_latitude.png", " (Excluding Florida)").map_err(|e| e.to_string())),
        ];
